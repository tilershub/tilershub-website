...
function ShopCard({ shop, index }) {
  ...
  return (
    <div
      style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 20, overflow: 'hidden', display: 'flex', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', transition: 'all 0.2s' }}
    ...
    >
      ...
      <div style={{ width: 110, flexShrink: 0, position: 'relative', minHeight: 140 }}>
        ...
        {shop.is_featured && (
          <div style={{ position: 'absolute', top: 8, left: 8, background: '#60A5FA', color: '#0F172A', fontSize: 8, fontWeight: 700, letterSpacing: 0.5, padding: '3px 7px', borderRadius: 20, boxShadow: '0 2px 6px rgba(0,0,0,0.25)', whiteSpace: 'nowrap' }}>
            ⭐ Top Rated
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, padding: '14px 16px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, fontWeight: 700, color: '#0F172A', lineHeight: 1.25, marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {shop.name}
        </div>
        {shop.description && (
        ...
        {chips.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
            {chips.map(s => (
              <span key={s} style={{ fontSize: 9, fontWeight: 600, padding: '2px 7px', borderRadius: 20, background: '#eef3fb', color: '#2563EB', border: '1px solid #d5e2f5' }}>{s}</span>
            ))}
          </div>
        )}
