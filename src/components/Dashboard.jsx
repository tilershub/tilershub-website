...
function ProviderDashboard({ user, claimedProfile, submission, showClaimedBanner, showWelcomeBanner }) {
  ...
  return (
    <div style={{ minHeight:'100dvh', background:'#FFFBF5', paddingBottom:80 }}>
      <style>{MOBILE_STYLES}</style>

      {/* ── Provider header ── */}
      <div style={{ background:'linear-gradient(135deg,#451A03 0%,#78350F 100%)', paddingBottom:0 }}>
              ...
              <div key={p.id} style={{ background:'#fff', borderRadius:16, border:'1px solid var(--border)', padding:'16px 18px', boxShadow:'var(--shadow-sm)' }}>
                ...
                <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                  ...
                  <button
                    ...
                    style={{ display:'inline-flex', alignItems:'center', gap:5, background: isSaved ? '#eef3fb' : '#FFFBF5', color: isSaved ? '#B45309' : 'var(--text-3)', border:`1px solid ${isSaved ? '#d5e2f5' : 'var(--border)'}`, borderRadius:8, padding:'7px 12px', fontSize:12, fontWeight:600, cursor:'pointer' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      {submittedBids.map(bid => {
        ...
        return (
          <div key={bid.id} style={{ padding:'16px 18px', background:'#fff', borderRadius:14, border:`1.5px solid ${isNew ? '#fde68a' : 'var(--border)'}`, borderLeft:`4px solid ${isNew ? '#f59e0b' : '#EDE4D3'}`, boxShadow:'var(--shadow-sm)' }}>
            ...
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10, marginBottom:6 }}>
              ...
              <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background: isNew ? '#fef3c7' : '#F7EFE2', color: isNew ? '#92400e' : '#57534E', whiteSpace:'nowrap', flexShrink:0 }}>{statusLabel}</span>
            </div>
            {bid.message && <p style={{ fontSize:13, color:'#44403C', lineHeight:1.6, margin:0 }}>{bid.message.length > 200 ? bid.message.slice(0,200)+'…' : bid.message}</p>}
          </div>
        )
      })}
    </div>
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.id} style={{ background:'#fff', borderRadius:14, border:'1px solid var(--border)', padding:'14px 16px', display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ width:48, height:48, borderRadius:12, background:'#B45309', flexShrink:0, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, fontWeight:700, color:'#fff' }}>
              {p.profile_image
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:11, color:'var(--text-3)', marginTop:2, display:'flex', flexWrap:'wrap', gap:'0 8px' }}>
                {rating > 0
                  ...
                  : <span style={{ color:'#A8A29E' }}>⭐ New · No reviews yet</span>
                }
    ...
    <div style={{ minHeight:'100dvh', background:'#FFFBF5', paddingBottom:80 }}>
      ...
      <div style={{ background:'#fff', borderBottom:'1px solid var(--border)' }}>
        <div className="db-header-pad" style={{ maxWidth:800, margin:'0 auto', padding:'20px 16px 0' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:10 }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div className="db-avatar" style={{ width:52, height:52, borderRadius:14, background:'var(--terra-50)', border:'2px solid rgba(180,83,9,0.2)', color:'var(--terra)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, fontWeight:700, flexShrink:0 }}>
                {initials}
              </div>
              <div>
  ...
  return (
    <div style={{ ...(full ? { minHeight:'60vh' } : { padding:40 }), display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ textAlign:'center' }}>
        <div className="spinner" style={{ margin:'0 auto 10px', borderColor:'rgba(120,53,15,0.15)', borderTopColor:'var(--navy)' }} />
...
function BidsPanel({ projectBids }) {
  ...
  if (!projectBids?.length) return (
    <div style={{ marginTop:12, padding:'12px 14px', background:'#FFFBF5', borderRadius:10, fontSize:12, color:'#A8A29E' }}>
  ...
  return (
    <div style={{ marginTop:12 }}>
      <button onClick={() => setOpen(o => !o)} style={{ display:'flex', alignItems:'center', gap:8, background:'none', border:'none', cursor:'pointer', padding:0, fontSize:13, fontWeight:700, color:'#B45309' }}>
        <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', background: newCount > 0 ? '#f59e0b' : '#EDE4D3', color: newCount > 0 ? '#fff' : '#57534E', borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:700 }}>
          {projectBids.length}
        </span>
        ...
        <span style={{ fontSize:14, color:'#A8A29E' }}>{open ? '▲' : '▼'}</span>
      </button>
      ...
      {open && (
        <div style={{ marginTop:12, display:'flex', flexDirection:'column', gap:10 }}>
          {projectBids.map(bid => {
            ...
            const cardStyle = { padding:'14px 16px', background:'#fff', borderRadius:12, border:`1.5px solid ${bid.status==='new'?'#fde68a':'#EDE4D3'}`, borderLeft:`4px solid ${bid.status==='new'?'#f59e0b':'#EDE4D3'}`, ...(profileUrl ? { cursor:'pointer', textDecoration:'none', color:'inherit', display:'block' } : {}) }
            return (
              <CardTag
                key={bid.id}
              ...
              >
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10, marginBottom:8 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0 }}>
                    <div style={{ flexShrink:0 }}>
                      {prov?.profile_image
                        ? <img src={prov.profile_image} alt={bid.bidder_name} style={{ width:44, height:44, borderRadius:'50%', objectFit:'cover', border:'2px solid #EDE4D3' }} />
                        : <div style={{ width:44, height:44, borderRadius:'50%', background: profileUrl ? '#B45309' : '#EDE4D3', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, color: profileUrl ? '#fff' : '#57534E', fontWeight:700 }}>{(bid.bidder_name||'?')[0].toUpperCase()}</div>
                      }
                    </div>
                    <div style={{ minWidth:0 }}>
                      <div style={{ fontSize:13, fontWeight:700, color: profileUrl ? '#B45309' : '#1C1917' }}>{bid.bidder_name}{profileUrl ? ' ↗' : ''}</div>
                      <div style={{ fontSize:11, color:'#57534E', marginTop:2, display:'flex', flexWrap:'wrap', gap:'0 8px' }}>
                ...
                {bid.message && <p style={{ fontSize:12, color:'#44403C', lineHeight:1.6, margin:'0 0 10px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                  ...
                  {profileUrl && <span style={{ fontSize:11, color:'#A8A29E' }}>Tap card to view profile ↗</span>}
                </div>
              </CardTag>
            )
          })}
...
function ProjectsTab({ projects, bids, isProvider }) {
  const STATUS_COLOR = {
    ...
    completed:      { bg:'#F3F4F6', color:'#44403C', label:'Completed'     },
  }
