    ...
    <div style={{ fontFamily: 'Montserrat, system-ui, sans-serif' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          <h2 style={{ fontSize: 22, fontWeight: 800, color: '#1C1917', margin: 0 }}>📣 Social Hub</h2>
          <span style={{ fontSize: 11, fontWeight: 700, background: GOLD, color: '#1C1917', padding: '3px 10px', borderRadius: 20 }}>NEW</span>
        </div>
      ...
      {/* Sub-tab nav */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 28, borderBottom: '2px solid #F7EFE2', paddingBottom: 0 }}>
        {SUBTABS.map(t => (
          <button key={t.key} onClick={() => setSubtab(t.key)}
            style={{
              ...
              color: subtab === t.key ? NAVY : '#A8A29E', transition: 'all 0.12s',
            }}>
            {t.label}
            <span style={{ fontSize: 10, fontWeight: 400, color: '#A8A29E', display: 'block', marginTop: 1 }}>{t.desc}</span>
          </button>
        ))}
      </div>
